...
Examples:
  speaks-for -c user123.p12 -f p12 -p 123456 -t yourepm.pem                                                        Generate a speaks-for credential which delegates access to YourEPM tool during 120 days. In this case the signing
                                                                                                                   credential is a PKCS#12 container
  speaks-for --credential user123.pem --format pem --password 123456 --toolcertificate yourepm.pem --duration 365  The Fed4FIRE user credential is PEM formatted, and access is delegated during 1 year
  speaks-for -vv -c user123.p12 -f ppem -p 123456 -t yourepm.pem -d 365 -o s4cred.base64                           Same command as previous one, but with DEBUG verbosity and storing the result on an output file
...
Examples:
  validate-speaks-for -s s4cred.base64  Validate a speaks-for credential and checks signature validi
